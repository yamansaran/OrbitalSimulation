public class HWM14Integration {
    ...
    public double[] getWindsWithTimeout(int iyd, double sec, double alt, double glat, double glon, double ap) {
    System.out.println("DEBUG: About to call native HWM14...");
    System.out.flush(); // Force immediate output
    
    try {
        double[] result = getWinds(iyd, sec, alt, glat, glon, ap);
        System.out.println("DEBUG: Native HWM14 call completed successfully");
        return result;
    } catch (Exception e) {
        System.out.println("DEBUG: Native HWM14 call failed: " + e.getMessage());
        throw e;
    }
}
}